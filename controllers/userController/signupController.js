...
const postSignup = async (req, res) => {
  try {
    ...
    // Schedule deletion exactly when OTP expires
    const delay = newUser.otpExpiresAt - Date.now();
    ...
    if (delay > 0) {
      setTimeout(async () => {
        ...
        if (!u.isverified && Date.now() > u.otpExpiresAt) {
          await userSchema.deleteOne({ _id: u._id });
          await Wallet.deleteOne({ user: u._id });
    ...
    res.json({
      success: true,
      message:
        "OTP sent successfully. Your redeem code will be activated after verification.",
      email,
    });
  } catch (error) {
  ...
  passport.authenticate(
    ...
    async (err, profile) => {
      try {
        if (err || !profile) {
          return res.redirect(
            "/login?message=Authentication failed&alertType=error"
          );
        }
